...
try {
    $result = $apiInstance->findAllPayments($page, $per_page, $external_customer_id, $invoice_id, 'application/json', $payment_status, $payment_statuses, $amount_from, $amount_to, $receipt_number, $created_at_from, $created_at_to, $payment_provider_type, $payment_method_type, $currency, $invoice_number, $payment_type, $payable_type, $search_term);
    print_r($result);
} catch (Exception $e) {
