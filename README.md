...
```php
$payments = new Lago\LagoPhpClient\Api\PaymentsApi(null, $config);
$result = $payments->findAllPayments(
    payment_status: ['succeeded', 'failed'],
    currency: 'EUR',
    amount_from: 5000000000,
    created_at_from: new DateTimeImmutable('2026-09-01')
);
$customers = new Lago\LagoPhpClient\Api\CustomersApi(null, $config);
$customerResult = $customers->findAllCustomerPayments(
...
Filters combine with AND and array entries with OR. Amount bounds are inclusive
integer cents (use 64-bit PHP for values above 2,147,483,647). Receipt and invoice
numbers match exactly, ignoring case. Date bounds include both days in the
organization timezone. See the generated method documentation for all options.
The existing fifth positional `contentType` argument remains supported.

To regenerate only these operations from an updated bundled spec:

```sh
python3 scripts/generate-payment-filters.py ../lago-openapi/openapi.yaml
php scripts/fix-generated-tests.php
vendor/bin/phpunit
```

This uses OpenAPI Generator 7.14.0, retains unrelated generated models/operations,
and applies reproducible fixes for date-only parameters, positional compatibility
and duplicate generated webhook test names.
